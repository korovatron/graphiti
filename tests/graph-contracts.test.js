...
async function assertExplicitExponentExitUsesImplicitMultiplication(page) {
    const result = await page.evaluate(() => {
        ...
        return {
            ...
            virtualKeyboardTapExitThenDigit: (() => {
                field.setValue('y=x');
                field.focus();
                field.executeCommand(['insert', '#@^{#?}']);
                field.insert('2');

                // Simulate explicit exit via tap/click outside superscript area
                // within the same field on touch devices.
                field.executeCommand('moveToNextChar');
                field.dispatchEvent(new PointerEvent('pointerup', { bubbles: true, pointerType: 'touch' }));
                field.dispatchEvent(new Event('touchend', { bubbles: true }));
            ...
            virtualKeyboardTapExitThenDigitReplacementInputType: (() => {
                field.setValue('y=x');
                field.focus();
                field.executeCommand(['insert', '#@^{#?}']);
                field.insert('2');
            ...
            virtualKeyboardPowerTemplateThenDigit: (() => {
                ...
                // Mirrors the x^n virtual key insertion path.
                field.executeCommand(['insert', '#@^{#?}']);
                field.dispatchEvent(new Event('selection-change', { bubbles: true }));
                ...
                return {
                    value,
                    parsed: parsedFor(value),
                    yAtThree: graphiti.evaluateFunction(value, 3)
                };
            })()
        };
    });
    ...
    assert(!result.virtualKeyboardTapExitThenDigit.value.includes('*'), `virtual-keyboard tap-exit then digit should avoid visible multiplication symbols: ${JSON.stringify(result)}`);
    assert(result.virtualKeyboardTapExitThenDigit.value.includes('\u2062') || result.virtualKeyboardTapExitThenDigit.value.includes('⁢'), `virtual-keyboard tap-exit then digit should include invisible-times separator: ${JSON.stringify(result)}`);
    assert.strictEqual(result.virtualKeyboardTapExitThenDigit.parsed, 'y=x^2*3', `virtual-keyboard tap-exit then digit should parse as implicit multiplication: ${JSON.stringify(result)}`);
    ...
    assert(
        ...
        `virtual-keyboard x^n template with exponent digit should parse as y=x^2 (equivalent grouped form allowed): ${JSON.stringify(result)}`
    );
    assert(approxEqual(result.virtualKeyboardPowerTemplateThenDigit.yAtThree, 9, 1e-9), `virtual-keyboard x^n template should evaluate as y=x^2 at x=3: ${JSON.stringify(result)}`);
}
